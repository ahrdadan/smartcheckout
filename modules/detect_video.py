...
import dataframe
from modules import helper, settings,detect_by_frame
import numpy as np
...
def detect_video(confidence):
    ...
    if st.button('Detect Video', use_container_width=True, disabled=st.session_state.get("btn1", True)):
        ...
        target_path= f"{uuid.uuid4()}.mp4"
        output_target_path=f'output/{target_path}'

        detect_by_frame.detect_video(name_video_path,output_target_path,model,confidence)
        create_data_to_output(target_path, datetime.now())
        
        encode_video(output_target_path,output_video_path)
...
def delete_output_video():
    ...
    if data["results"]:
        for item in data["results"]:
            ...
            if difference_in_seconds > settings.TIME_DELETE_SAVED_VIDEO:
                # remove item here
                os.remove(f'output/{item["name"]}')
